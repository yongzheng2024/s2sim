...
# 读取 Excel 文件
# df = pd.read_excel(r'D:\Project\Draw\comparison-Time.xlsx', sheet_name='Sheet1')
df = pd.read_excel(os.path.join(current_dir, 'comparison-Time.xlsx'))
...
for i, value in enumerate(cpr_time_2):
    if value == 0:
        ...
        # 绘制一个长柱子
        ax.bar(x_pos, 700000, width=bar_width, color=color2, hatch=hatch_pattern2)
        # 在柱体上方添加 ">24h" 标记
        ax.text(x_pos, 0.96, '>2h', ha='center', va='bottom', fontsize=8, color=color_red, rotation=0, transform=ax.get_xaxis_transform(), fontweight='bold')
...
ax.legend(prop={'size': 13, 'weight': 'bold'}, loc='upper left')
plt.yscale('log')
...
# plt.savefig(r'D:\Project\Draw\eval-comparison.pdf')
plt.savefig(os.path.join(current_dir, 'eval-comparison.pdf'))
